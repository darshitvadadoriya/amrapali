...
var link_port = window.location.origin;
var company_link = `${link_port}/api/resource/Company?limit=null`;
fetch(company_link)
    .then(r => r.json())
    .then(data => {
        let response_data = data.data;
        let selectElement = document.getElementById('company_selection');
        response_data.forEach(company => {
    ...
    .catch(error => {
        alert("No companies are present at the moment.")
    });
...
login.bind_events = function () {
	...
	$(".form-login").on("submit", function (event) {
		...
		args.device = "desktop";
		// settigs email_id and company on localstorage
		let email_div = document.getElementById('login_email').value
		let company_select = document.getElementById('company_selection');	
		let login_dets = {
			"email_id" : email_div,
			"company" : company_select.value
		}
		let login_dets_json = JSON.stringify(login_dets);
		localStorage.setItem('Details', login_dets_json);
		data = JSON.parse(localStorage.getItem("Details"))

		var user = data.email_id;
		var company = data.company;
		company_name = company;
		fetch(`${link_port}/api/method/envision_accounts.api.get_users_with_role?user_name=${user}`, {
			method: 'GET',
			headers: {
			...
			if(response.message && response.message.length > 0){	
				console.log("Success", response.message[0].name);
			}
			else{
				generate_user_permissions(link_port,user,company)
			}
		}).catch(error => {
...
function generate_user_permissions(link_port,user,company){
	get_url = `${link_port}/api/resource/User Permission?limit_page_length=None&fields=["name","allow","user","for_value"]&filters=[["user","=","${user}"],["allow","=","Company"]]`;
		fetch(get_url, {
			method: "GET",
			headers: {
				"Content-Type": "application/json",
		...
		.then(data => {
			if(!data.data.length){
				let post_url = `${link_port}/api/resource/User%20Permission?limit=null`
				fetch(post_url, {
					method: "POST",
					headers: {
						"Content-Type": "application/json",
				...
				.then(r => r.json())
				.then(data => {
						console.log(`The data: ${user} and ${company}`);
				})
					.catch(error => console.error('Error:', error));
			}
			if(data.data.length != 0){
				let put_url = `${link_port}/api/resource/User%20Permission/${data.data[0].name}`;
				fetch(put_url,{
					method: "PUT",
					headers: {
						"Content-Type": "application/json",
