...
class StockTransfer(Document):
	def on_submit(self):
				...
				for data in self.items:
					...
					row.location = self.from_location
	...
	def on_cancel(self):
				...
				balance_out = frappe.db.get_all("Stock Balance Out", fields=["*"], filters={"record_doc": self.name})

				# stock balance in revet back process
				for balance in balance_in:
					# check if matching row in stock summary table with same location
